...
/**
 * envoy_dynamic_module_type_stat_sink_envoy_ptr is a raw pointer to
 * the DynamicModuleStatsSink class in Envoy. Valid only during flush/histogram callbacks.
 *
 * OWNERSHIP: Envoy owns the pointer.
 */
typedef void* envoy_dynamic_module_type_stat_sink_envoy_ptr;

/**
 * envoy_dynamic_module_type_stat_sink_snapshot_envoy_ptr is a raw pointer to the
 * MetricSnapshot object passed during flush.
 *
 * OWNERSHIP: Envoy owns the pointer. Valid only during the flush event callback.
 */
 ...
 * The module can iterate over counters, gauges, histograms, and text readouts via the provided
 * callbacks.
 *
 * This is called on the stats flush thread. Implementations must not block.
 *
 * @param config_module_ptr is the pointer to the in-module configuration object.
 * @param sink_envoy_ptr is the pointer to the DynamicModuleStatsSink object in Envoy.
 * @param snapshot_envoy_ptr is the pointer to the MetricSnapshot. Valid only during this call.
 */
...
/**
 * envoy_dynamic_module_on_stat_sink_on_histogram_complete is called synchronously when a
 * histogram observation is recorded. Implementations must be thread-safe.
 *
 * @param config_module_ptr is the pointer to the in-module configuration object.
 * @param histogram_name is the name of the histogram.
